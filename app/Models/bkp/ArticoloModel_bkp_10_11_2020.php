...
class ArticoloModel extends Model
{
    ...
    public function ottieniInfoItemAffidato($idAffidamento){
        // $builder = $this->db->table('tb_affidamento')->select('*')->join('tb_articolo','tb_affidamento.fk_articolo_id = tb_articolo.articolo_id')->where('affidamento_id = ', $idAffidamento);
        // return  $builder->get()->getResultArray();
            // $query = $this->db->query("SELECT * 
            // FROM int_magazzino.tb_affidamento INNER JOIN tb_articolo ON tb_affidamento.fk_articolo_id = tb_articolo.articolo_id
            // LEFT JOIN tb_tipologia_articolo ON tb_articolo.fk_tipologia_articolo_id = tb_tipologia_articolo.tipologia_articolo_id
            // LEFT JOIN tb_condizione ON tb_condizione.condizione_id = tb_affidamento.fk_condizione_id
            // LEFT JOIN tb_fornitore ON tb_fornitore.fornitore_id = tb_articolo.fk_fornitore_id WHERE affidamento_id = ".$idAffidamento);
    ...
    {
        //$builder = $this->db->table('v_lista_articoli_presenti_completa')->where('fk_tipologia_articolo_id = ',$id);
    ...
    public function inserisciAttributi($fk_tipologia_articolo_id, $descrizione)
    {
        ...
        // if ($builder->countAllResults() > 0) {
        //     return "Attenzione, risulta già presente una attributo con questo nome";
        // } else {
            $data = [
                //'attributo_id' => rand(),
    ...
    public function inserisciAttributiValore($fk_tipologia_articolo_id, $descrizione)
    {
        ...
        // $builder = $this->db->table('tb_attributo_valore')->where('descrizione = ', $descrizione);
        // if ($builder->countAllResults() > 0) {
        //     return "Attenzione, risulta già presente l'attributo con questo valore";
        // } else {
            $data = [
                //'attributo_id' => rand(),
    ...
    public function getListaAffidi($anagrafica_id, $tipoAffido, $attivo)
    {
        ...
        if (empty($anagrafica_id)) {
            $builder = $this->db->table('v_lista_affidamento')->select("*")->where('affidamento_tipo','A');
            return  $builder->get()->getResultArray();
        } else {
    ...
    public function getSchedaCarburante(){
        ...
        $builder = $this->db->table('tb_consumi_carburante')->select("scheda_carburante")->distinct(); 
    ...
    public function getSchedeCarburanti($scheda_carburante,$id_nominativo_risorsa,$mese,$anno){
        ...
        //echo "SELECT * FROM tb_consumi_carburante WHERE ".$valore;
        ...
        //echo $valore; 
        // $builder = $this->db->table('tb_consumi_carburante')->select("*")->where('scheda_carburante = ',$scheda_carburante); 
        // $row = $builder->get()->getResultArray();
        // return $row;
    }
    ...
    public function ricercaMateriale($seriale,$tipo,$stato,$condizione,$commessa,$sede){
        ...
        $valore = substr($valore,0,-4);
        //echo "SELECT * FROM v_lista_articoli_presenti_completa WHERE ".$valore;
    ...
    public function ricercaModuli($seriale,$tipologia_documento,$data_movimento,$commessa,$id_operazione,$stato_documenti,$operatore){
        //echo "stato_documenti: ".$stato_documenti."<br>";
    ...
    public function richiestaSpostamento($data){
        ...
        //$id_responsabile = $this->getReponsabile($data['riferimento_commessa_output']);
        //print_r($data);
        $cntArray = count($data); 
        //echo "Elementi: ".$cntArray; 

        // if ( !in_array("sposta",$data) || !in_array("resta",$data) ){
        if ( $cntArray == 10  ){
            ...
            foreach($data as $valore){
                if ( strpos($valore,';') !== false ){
                    ...
                    //Aggiorno lo stato dell'item da A a P
                    $dataUpdate = [
                       "affidamento_tipo" => 'P'
                    ];
        ...
        if ($this->db->affectedRows() >= 0) {
            ...
            /* Devo verificare se 
                Utente loggato == rif.commessa attuale == rif.commesso spostamento 
                se è vero allora no email 
            */
            ...
            if ( $esito_richiesta_invia_email ){
                ...
                for($i=0; $i<count($movimento); $i++){
                    $tabella .= "<tr>
                                <td style='border-bottom: 1px solid #ddd; text-align:center;'>".date('d/m/Y H:i:s',strtotime($movimento[$i]['movimento_data_richiesta']))."</td>
                                <td style='border-bottom: 1px solid #ddd; text-align:center;'>".date('d/m/Y',strtotime($movimento[$i]['movimento_data_inizio_servizio']))."</td>
                                <td style='border-bottom: 1px solid #ddd; text-align:center;'>".strtoupper($movimento[$i]['nominativo_spostamento'])."</td>
                                <td style='border-bottom: 1px solid #ddd; text-align:center;'>".strtoupper($movimento[$i]['magazzino_provenienza_dsc'])." - ".strtoupper($movimento[$i]['magazzino_destinazione_dsc'])."</td>
                                <td style='border-bottom: 1px solid #ddd; text-align:center;'>".strtoupper($movimento[$i]['commessa_provenienza'])." - ".strtoupper($movimento[$i]['commessa_destinazione'])."</td>
                                <td style='border-bottom: 1px solid #ddd; text-align:center;'>".strtoupper($movimento[$i]['articolo_seriale'])." - ".strtoupper($movimento[$i]['descrizione'])."</td>
                                <td style='border-bottom: 1px solid #ddd; text-align:center;'>".strtoupper($movimento[$i]['movimento_stato'])."</td>
                            </tr>";
        
                }
                $tabella .= "</table>";
                            
                $this->inviaEmail($email,$tabella,$nominativo_responsabile_commessa,"SPOSTAMENTO"); 
                */
    ...
    public function storico_movimenti($fk_articolo_id){
        // $query = $this->db->query('SELECT t1.affidamento_id, t1.affidamento_data, t1.affidamento_tipo,t1.affidamento_note,affidamento_modulo,CASE WHEN affidamento_modulo = 0 THEN "No" ELSE "Si" END AS affidamento_modulo_descrizione,data_caricamento_modulo,CONCAT(t2.nome," ",t2.cognome) as nominativo_assegnatario, CONCAT(t3.nome," ",t3.cognome) as nominativo_destinatario, descrizione as descrizione_commessa, settore as settore_commessa
        // FROM tb_affidamento as t1 LEFT JOIN tb_anagrafica as t2 ON t1.fk_utente_assegnatario_id = t2.anagrafica_id
    ...
    public function richieste_movimento(){
        $infoArray = array(); 
        
        $builder = $this->db->table('v_richieste_movimento')->select("*")->where('movimento_tipo','P')->groupBy('richiesta_id');
    ...
    public function getRichiestePending(){
    ...
    public function confermaSpostamento($id){
        ...
        if ( $row[0]['fk_affidamento_id'] != 0 ){
            ...
            //echo "No item";
            //echo "FK_COMMESSA_DESTINAZIONE:".$row[0]['fk_commessa_destinazione']."<br>fk_anagrafica_destinatario_id:".$row[0]['fk_anagrafica_destinatario_id'];
            //Significa che non ci sono item assegnati alla risorsa
        ...
        //     $tabella .= "</table>";
        // }
        // $this->inviaEmail($email,$tabella,$nominativo_responsabile_commessa,"CONSENTI_SPOSTAMENTO"); 

        //procedo ad inviare l'email per la notifica dello spostamento
    ...
    public function confermaRifiuto($id){
        ...
        //     $tabella .= "</table>";
        // }
        // $this->inviaEmail($email,$tabella,$nominativo_responsabile_commessa,"RIFIUTO_SPOSTAMENTO"); 
    }
